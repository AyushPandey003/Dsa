...
public:
    int findClosest(int arr[], int n, int k) {
        ...
        // At this point, left is the index of the smallest element greater than k
        // and right is the index of the largest element smaller than k
        int closest;
        if (left >= n) {
            closest = arr[right];
        } else if (right < 0) {
            ...
            int diff_left = abs(arr[right] - k);
            int diff_right = abs(arr[left] - k);
            ...
            if (diff_left < diff_right) {
                ...
                closest = arr[left]; // When diff_left == diff_right, choose the larger value which is arr[left]
            }
        }
...
int main() {
    ...
    int n1 = 4;
    int k1 = 4;
    cout << sol.findClosest(arr1, n1, k1) << endl; // Output: 3

    int arr2[] = {1, 2, 3, 5, 6, 8, 9};
    int n2 = 7;
    int k2 = 4;
    cout << sol.findClosest(arr2, n2, k2) << endl; // Output: 5
